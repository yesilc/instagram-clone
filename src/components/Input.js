    ...
    return (
        <label className='block relative flex bg-zinc-50 border rounded-sm focus-within:border-gray-400'>
            <input type={inputType} className={classNames({
                ...
                "pt-[10px]":field.value
                })}{...field} {...props} /> {/*props inputtaki değerler */}
            ...
            {type === 'password' && field.value && ( /*props?.value -> propslar bazen gelmeyebiliyor ekrana hata fırlatmasın diye öncelikle kontrol amaçlı ? konuluyor */
                <div onClick={() => setShow(show => !show)} className="h-full cursor-pointer select-none flex items-center bg-zinc-50 text-sm font-semibold pr-2">
                    {show ? 'Gizle' : 'Göster'}
                </div>
            )}
        </label>
